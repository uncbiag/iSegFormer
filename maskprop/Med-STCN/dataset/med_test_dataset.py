...
class MedTestDataset(Dataset):
    def __init__(self, root, imset='val.txt', resolution=480, single_object=False, target_name=None, valid_obj_labels=(5,6), is_rib=False):
        self.root = root
        if resolution == 480:
            res_tag = '480p'
        else:
        ...
        if is_rib:
            self.mask_dir = path.join(root, 'Annotations_rib', res_tag)
            self.mask480_dir = path.join(root, 'Annotations_rib', res_tag)
        else:
            self.mask_dir = path.join(root, 'Annotations', res_tag)
            self.mask480_dir = path.join(root, 'Annotations', res_tag)
        ...
        self.shape = {}
        self.size_480p = {}
        self.valid_obj_labels = valid_obj_labels
    ...
    def __getitem__(self, index):
        ...
        for f in range(self.num_frames[video]):
            img_file = path.join(self.image_dir, video, '{:07d}.jpg'.format(f))
        ...
        # set invalid labels to the background
        masks[np.logical_and.reduce([masks != label for label in self.valid_obj_labels])] = 0
        ...
        if self.single_object:
            ...
            labels = np.array(self.valid_obj_labels)
