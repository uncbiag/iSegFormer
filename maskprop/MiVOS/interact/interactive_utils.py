...
def load_volume(path, normalize=True, min_side=None):
    image = sitk.ReadImage(path)
    image_npy = sitk.GetArrayFromImage(image)

    image_npy_rgb = np.stack([image_npy, image_npy, image_npy], axis=0)
    image_npy_rgb = np.transpose(image_npy_rgb, (1,2,3,0))
